...
def create_model_loader(model_path: str, model_name: str = "image-classifier") -> ModelLoader:
    """
    根据模型文件扩展名创建对应的模型加载器
    ...
    elif ext in ['.mph', '.pkl', '.pickle', '.pt', '.pth']:
        logger.info(f"检测到自定义模型格式: {ext}")
        logger.warning("需要根据实际模型格式修改CustomModelLoader的加载逻辑")
        return CustomModelLoader(model_path, model_name)
    else:
        logger.warning(f"未知模型格式: {ext}, 使用自定义加载器")
        return CustomModelLoader(model_path, model_name)
